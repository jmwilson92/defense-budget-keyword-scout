...
Full version with Upload button only in Data Ingestion
"""
...
with tab1:
    ...
    with col_b:
        max_pdfs = st.number_input("Max PDFs to process (0 = all)", min_value=0, value=0, step=1, help="Useful for testing")
    ...
    if not index_exists or ix is None:
        ...
        if st.button("🚀 Find Best Matches for My Capabilities", type="primary"):
            with st.spinner("Scoring all indexed sections against your keywords... (can take 10-60s depending on index size)"):
                all_docs = get_all_indexed_documents(ix) if 'get_all_indexed_documents' in dir() else []
...
with tab4:
    ...
    so you can quickly find Program Managers, Technical POCs, Contracting Officers, or related opportunities.
    """)
...
with tab5:
    ...
    **Features:**
    ...
    st.caption("v2.6 • Upload only • May 2026")
